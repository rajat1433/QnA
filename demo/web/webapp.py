...
def init(data_dir, model_file):
    ...
    test_data_path = glob.glob('%s/qa17_positional-reasoning_test.txt' % memn2n.data_dir)

    print(test_data_path)
    print(len(memn2n.general_config.dictionary)) 
    print(len(memn2n.reversed_dict)) 
...
def get_story():
    ...
    story_txt, question_txt, correct_answer = memn2n.get_story_texts(test_story, test_questions, test_qstory,question_idx, story_idx, last_sentence_idx)
    # Format text
    story_txt = "\n".join(story_txt)
    question_txt += "?"
